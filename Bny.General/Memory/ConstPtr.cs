...
/// <summary>
/// Read only version of Ptr. Represents a countinuous region of read only
/// memory. Same as ReadOnlySpan,
/// but with pointer arithmetic operator overloads.
/// </summary>
...
public readonly ref struct ConstPtr<T>
{
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public ConstPtr(ReadOnlySpan<T> span)
    ...
    /// <returns
    /// >New ConstPtr over the specified range of the original ConstPtr
    /// </returns>
    /// <exception cref="IndexOutOfRangeException">
    /// Thrown when the requested slice is out of the range
    /// </exception>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public ConstPtr<T> Slice(int start, int length)
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static implicit operator ReadOnlySpan<T>(ConstPtr<T> ptr)
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static ConstPtr<T> operator ++(ConstPtr<T> ptr)
        => ptr._length == 0 
            ? throw new IndexOutOfRangeException()
            : new(ref ptr.At(1), ptr._length - 1);

    /// <summary>
    /// Determines whether the given ptr is not empty
    ...
    /// <summary>
    /// Determines whether the given ptr is empty
    /// (has the same effect as ptr.Length == 0)
    /// </summary>
    ...
    /// <summary>
    /// Adds the given number to the ConstPtr
    /// ('ptr + 5' has the same effect as 'ptr[5..]')
    /// </summary>
    ...
    /// <exception cref="IndexOutOfRangeException">
    /// Thrown when the added value is larger than the ptr.Length
    /// </exception>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static ConstPtr<T> operator +(ConstPtr<T> ptr, int value)
...
#pragma warning disable CS8500
    ...
    /// <summary>
    /// Determines the number of elements bethween the given pointers that
    /// point to the same continuous block of memory. If the pointers not't
    /// point to the same memory, the behaviour is undefined.
    /// </summary>
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static unsafe int operator -(ConstPtr<T> p1, ConstPtr<T> p2)
...
#pragma warning restore CS8500
    ...
    /// </exception>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static T operator +(ConstPtr<T> ptr)
        => ptr._length == 0
            ? throw new IndexOutOfRangeException()
            : ptr._ptr;

    /// <summary>
    /// Determines whether the two pointers point to the same memory and have
    /// the same length. (This doesn't check the data itself)
    /// </summary>
    /// <param name="ptr1">Pointer to be compared with ptr2</param>
    /// <param name="ptr2">Pointer to be compared with ptr1</param>
    /// <returns>True if the two pointers point to the same memory and have
    /// the same length</returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static bool operator ==(ConstPtr<T> ptr1, ConstPtr<T> ptr2)
        => ptr1._length == ptr2._length &&
           Unsafe.AreSame(ref ptr1._ptr, ref ptr2._ptr);

    /// <summary>
    /// Determines whether the two pointers don't point to the same memory or
    /// don't have the same length (This doesn't check the data itself)
    /// </summary>
    /// <param name="ptr1">Pointer to be compared with ptr2</param>
    /// <param name="ptr2">Pointer to be compared with ptr1</param>
    /// <returns>
    /// True if the two pointers don't point to the same memory or
    /// don't have the same length, otherwise false
    /// </returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static bool operator !=(ConstPtr<T> ptr1, ConstPtr<T> ptr2)
        => ptr1._length != ptr2._length ||
    ...
    /// </summary>
    /// <returns>
    /// Enumerator that enumerates all the data in the memory
